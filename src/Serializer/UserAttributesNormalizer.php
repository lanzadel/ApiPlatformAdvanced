...
class UserAttributesNormalizer implements ContextAwareNormalizerInterface, SerializerAwareInterface
{
    ...
    const USER_ATTRIBUTE_NORMALIZER__ALREADY_CALLED = 'USER_ATTRIBUTE_NORMALIZER__ALREADY_CALLED';
    ...
    {
        if(isset($context[self::USER_ATTRIBUTE_NORMALIZER__ALREADY_CALLED])) {
            return false;
        }
    ...
    private function passOn($object, $format, $context)
    {
        if(!$this->serializer instanceof NormalizerInterface) {
            throw new \LogicException(sprintf('cannot normalize object "%s" because the injected serializer is not a normalize', $object));
        }
        $context[self::USER_ATTRIBUTE_NORMALIZER__ALREADY_CALLED] = true;
